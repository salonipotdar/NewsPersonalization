# generate matrix factorization data from original training data, so it will do user freq threshold filter inside
# user_id doc_id ctr
# user_id is mapped to local id start from 0, so are doc_id
# output:
# mf training data; user_id_map, doc_id_map
# python program input_training_data user_freq_file user_freq_threshold output_training_data user_id_map_file doc_id_map_file
...
for uid, value in user_ctr.items():
	...
	if ctr > 1e-10:
            fout_data.write("%d %d %g\n" % (user_localid[uid], doc_localid[doc], ctr))
